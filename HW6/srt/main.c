...
void * traceRays(void * threadNumber) {
    ...
    for( int px=*((int*)threadNumber); px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            // systematically enter values into value array, which will
	    // later be printed out systematically
...
main( int argc, char **argv )
{
    ...
    // this is necessary because a pointer is passed, as opposed
    // to the direct int variable. Due to this distinction
    // the given argument changes when the main thread
    // continues.
    int idStorage[nthreads];

    for (int i = 0; i < nthreads; i++) {
      idStorage[i] = i;
      int ret = pthread_create(&threads[i], NULL, traceRays, idStorage+i);
      if (ret) fprintf(stderr, "%i ", ret);
    }
    
    for (int i = 0; i < nthreads; i++)
      pthread_join(threads[i], NULL);

    // print results of the multithreaded function
