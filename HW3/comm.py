...
class comm:
    def __init__(self, printF1, printF2, printF3, sorted, args):
        ...
        for i in range(len(self.file1)):
            self.file1[i]=self.file1[i].strip()
            
        for i in range(len(self.file2)):
            self.file2[i]=self.file2[i].strip()

        return

    def unsortedCompare(self):
        spacingF2 = ""
        spacingF3 = ""
        ...
        for line1 in self.file1:
            ...
            for j in range(len(self.file2)):
                ...
                if (line1 == line2):
                    if (self.printF3 == True):
                        print(spacingF3+line1)
                        match = True
                    del self.file2[j]
                    break
                

            if match == False and self.printF1 == True:
                print(line1)

        for line2 in self.file2:
            if (self.printF2 == True):
                print(spacingF2+line2)
    ...
    def sortedCompare(self):
        ...
        while i < len(self.file1) or j < len(self.file2):
            ...
            if (line1 != None and line2 != None and line1 == line2):
                ...
                i += 1;
                j += 1;
            elif (line2 == None or (line1 != None and line1 < line2)):
                if self.printF1 == True:
                    print(self.file1[i])
                i += 1;
            elif (line1 == None or (line2 != None and line1 > line2)):
                if self.printF2 == True:
...
def main():
    ...
    parser.add_option("-3",
                      ...
                      dest="f3",
                      default=True,
                      help="supress column 3 (lines that appear in both files)");
    parser.add_option("-u",
                      action="store_false",
                      dest="sort",
                      default=True,
    ...
    try:
        ...
        parser.error("Error in Comparison")
